...
const searchGames = async(input) => {
    ...
    try {
        const response = await fetch(`https://api.rawg.io/api/games?ordering=-metacritic&key=${apiKey}&search=${input}&page_size=100`); 
...
const searchByLastMonth = async() => {
    try {
        const response =  await fetch(`https://api.rawg.io/api/games?ordering=-released&key=${apiKey}&date=2024-12-09&page_size=100`); 
        const data = await response.json();
        console.log(data)
        displayGames(data.results); }
...
const searchByLastWeek = async() => {
    try {
        const response =  await fetch(`https://api.rawg.io/api/games?ordering=released&key=${apiKey}&date=2024-12-02,2024-12-09&page_size=100`); 
        const data = await response.json();
        console.log(data)
        displayGames(data.results); }
...
const searchByBestYear = async() => {
    try {
        const response = await fetch(`https://api.rawg.io/api/games?ordering=-metacritic,-rating,-reviews_count&key=${apiKey}&dates=2023-12-09,2024-12-09&page_size=100`);
        const data = await response.json();
        console.log(data)
        displayGames(data.results); }
